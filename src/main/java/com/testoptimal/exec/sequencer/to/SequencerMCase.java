 ...
 *
 */
public class SequencerMCase implements Sequencer {
	private static Logger logger = LoggerFactory.getLogger(SequencerMCase.class);
	...
	private List<MCase> caseList = new java.util.ArrayList<>();
	private MCase curMCase;
	...
	public SequencerMCase (ExecutionDirector execDir_p) throws MBTAbort, Exception {
		this.execDir = execDir_p;
		this.scriptExec = this.execDir.getScriptExec();
		this.execSetting = this.execDir.getExecSetting();

		this.caseList = this.scriptExec.getMCaseMgr().getMCaseList((List<String>) this.execDir.getExecSetting().getOption("MCaseList"));
	...
	private List<SequencePath> genPaths () {
		List <SequencePath> retList = new java.util.ArrayList<>();
		for (MCase m: this.caseList) {
			List<Transition> travList = new java.util.ArrayList<>();
			State fromState = this.homeState;
			State toState;
			for (MStep s: m.getStepList()) {
				if (s.isState) {
					toState = this.networkObj.findStateByUID(s.toID);
					if (toState==null) toState = this.networkObj.findState(s.toID);
					if (toState==null) logger.warn("Unknown state id for MCase " + m.getName() + ": " + s.toID);
					if (s.naveTo) {
						travList.addAll(this.genTransPath(fromState, toState));
						fromState = toState;
					}
					else {
						fromState = toState;
					...
					Transition t = fromState.getTransition(s.toID);
					if (t==null) t = fromState.getTransByUID(s.toID); 
					if (t==null) {
						logger.warn("Unknown transition id for MCase " + m.getName() + ": " + s.toID);
					}
					travList.add(t);
					fromState = (State) t.getToNode();
				}
			}
			
			// if at last step, auto route to home state
			if (!fromState.isModelFinal()) {
				travList.addAll(this.genTransPath(fromState, this.homeState));
			}
			retList.add(new SequencePath(travList));
		}
		return retList;
	}

	public List<Transition> genTransPath (State fromState_p, State toState_p) {
		try {
			List<Transition> transList = SequencerPathBase.getShortestPathTransList(this.shortestPathObj, fromState_p.getId(), toState_p.getId());
			return transList;
		}
